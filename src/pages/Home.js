  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '2rem' }}>
        <button onClick={() => handleSelectType('blue')} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
          <FaHardHat size={60} color="#004080" />
          <p style={{ marginTop: '0.5rem', fontWeight: 'bold', color: '#004080' }}>{t('blue_collar')}</p>
        </button>

        <button onClick={() => handleSelectType('white')} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
